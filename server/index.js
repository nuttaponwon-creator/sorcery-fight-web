...
// Rooms structure: { "ROOM123": { players: {}, hostId: null, gameStarted: false } }
let rooms = {};
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (data) => {
        ...
        // NEW: Cleanup previous room if already in one
        if (socket.data.room) {
            const oldRoom = socket.data.room;
            if (rooms[oldRoom]) {
                delete rooms[oldRoom].players[socket.id];
                socket.leave(oldRoom);
                console.log(`[ROOM ${oldRoom}] ${name} left (moving to ${room})`);
                
                // Host migration for old room
        ...
        // Safety Guarantee: If only 1 player in this room, they MUST be host
        if (Object.keys(roomData.players).length === 1) {
            roomData.hostId = socket.id;
        }

        // Notify this player
        console.log(`[SERVER] Emitting roomInfo to ${socket.id} for room: ${room}`);
        ...
        console.log(`[ROOM ${room}] ${name} joined. Host Status: ${socket.id === roomData.hostId}`);
    });
    ...
    socket.on('playerAction', (data) => {
        const room = socket.data.room;
        if (room) {
            socket.to(room).emit('playerActed', { id: socket.id, ...data });
        }
    });
